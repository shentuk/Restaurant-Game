...
项目内图片已做压缩以加快加载。原图备份在 `images/original/`。如需重新压缩或恢复：

- 运行压缩：`npm run compress-images`（会跳过已备份的原图，仅覆盖当前图片）
- 恢复原图：从 `images/original/` 复制回 `images/` 即可
...
2. 只能使用附件中提供的图片资源，其他样式均使用 CSS 实现；如果有额外自己添加的需求场景，可以自行按需求增加图片资源；
3. 提供的原始图片资源体积较大，根据实际需要进行性能优化所需要的压缩；
4. 代码风格优雅，代码结构合理，技术设计恰到好处；
5. 框架使用方面，建议先不使用框架实现一遍；
6. 然后学习使用数据驱动思想，将游戏中的数据和状态都抽象出来，用数据来驱动游戏的行为和状态变化；
7. 接下来可以分别选用一些行业中的主流框架，包括css样式方面的库，以及react、vue等，并结合webpack等一些工程化方案；
8. 可以读一些《重构》之类的书籍，尝试做一些抽象与封装，然后进行一次重构，然后再次复习阅读后，再次重构；
9. 尝试进行组件的抽象与封装，结合看一下设计模式相关知识，尝试用不同的设计模式进行实践，理解不同设计模式在不同场景运用下的优劣，理解每种设计模式要解决的问题
